<template>
  <div class="recommended-products container">
    ...
    <div class="products-grid">
      <div class="product-card" v-for="product in activeProducts" :key="product.productId">
        <img :src="product.image" class="product-image" alt="商品圖片" />
        <div class="product-details">
          <h5 class="product-title">{{ product.productName }}</h5>
          <p class="product-price">價格: NT${{ product.price }}</p>
          <div class="action-buttons">
            <router-link :to="`/product/${product.productId}`" class="btn btn-view"> 查看詳細 </router-link>
            <!-- 商品卡片中的收藏按鈕 -->
